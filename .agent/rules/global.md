...
- **Default execution first**: For ordinary tasks, execute directly once the target and constraints are clear. Do not force a draft-review round before routine edits, small fixes, or straightforward file updates.
- **Draft review only for high complexity**: Use a pre-write draft plus explicit review only when the task is highly complex, high-risk, architecture-shaping, or spans many coordinated files or steps where review materially reduces risk.
- **Complex-request planning**: Only for highly complex requirements, first write a clear, specific, complete, and executable solution in English in `.draft`; during execution, update that plan in `.draft` in a timely manner based on actual progress and new findings. Do not create or update `.draft` for ordinary tasks.
