...
Polywise 管理着一个高优先级的长期记忆 (LTM) 和按时间顺序排列的日记，旨在解决神经网络的“近因偏见”问题。LTM 存储提炼后的身份和规则，而日记则提供时间维度的上下文。

- **长期记忆 (LTM)**：自动提炼主动性语句（身份、规则、约束）和核心概念模式。它具备基于决策的保留机制，在保持有限容量的同时保留高价值的联想。
- **日记 (Diary)**：在 **SLEEPING** 状态下自动生成会话摘要。条目按时间索引，允许系统“记住”跨越数天的时间线和事件流。
...
```bash
          用户输入 (Input)
                ↓
┌──────────────────────────────────────────────┐
│  阶段 0: Cortex 执行门控 (Executive Gating)  │
│  - 检查 cot_depth 参数                       │
│  - 初始化 ChainEmitter 和 WorkingMemory      │
└──────────────────────────────────────────────┘
      ↓                      ↘
...
│ - 习惯反应: 获取并处理习惯反应 (刺激节点)                                │
│ - 图谱回忆: 从记忆召回 (关键词 -> 节点 -> 激活扩散 -> 上下文)            │
│ - 外部搜索: 向量与全文搜索 (使用 Pipeline 进行嵌入)                      │
│ - 记忆搜索: 检索长期记忆 (LTM) 与日记 (Diary)                            │
└──────────────────────────────────────────────────────────────────────────┐
      ↓
┌──────────────────────────────────────────────────────────────────────────┐
│ 阶段 2: 结果聚合                                                         │
│ - aggregateResults: 合并召回、搜索、记忆与习惯                           │
│ - 数据去重与归一化                                                       │
└──────────────────────────────────────────────────────────────────────────┐
      ↓
┌──────────────────────────────────────────────────────────────────────────┐
│ 阶段 3: 重排序 (Reranking)                                               │
│ - Pipeline.rerank: 对知识 (Knowledge) 和行为 (Action) 候选进行语义评分   │
...
- 🎯 **[BGE Reranker v2-m3](https://huggingface.co/onnx-community/bge-reranker-v2-m3-ONNX)** - **精准排序**：对检索到的知识和行为进行重排序，确保最相关的内容优先呈现。
- 🧠 **[Qwen3-0.6B](https://huggingface.co/onnx-community/Qwen3-0.6B-ONNX)** - **推理与规划**：驱动 Cortex 的迭代规划和决策，处理复杂的“慢速路径”查询。
