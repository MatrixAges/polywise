...
<p align="center"><strong>你的 AI 数字分身</strong></p>
...
// cot       -> ChainEmitter (如果 cot_depth <= 1 则为 null)
```
...
cot?.on(event => {
	...
	if (event.memory[0]?.metadata) {
		console.log(`最相关描述: ${event.memory[0].metadata.desc}`)
	}
})
```
...
// 通过查询删除记忆（无需 memory_id）
await poly.forget({
	query: '关于用户偏好的过时信息'
})

// 删除指定记忆及通过查询找到的所有相关记忆
await poly.forget({
	memory_id: 'abc123',
	query: '关于用户偏好的过时信息'
})
...
- `memory_id`（可选）：要删除的记忆 ID
- `query`（可选）：自然语言查询，用于查找并删除相关记忆
- **注意**：`memory_id` 和 `query` 至少需要提供一个
