...
import { BundledLanguage, bundledLanguages, BundledTheme, createHighlighter, Highlighter } from 'shiki'
...
export function loadHighlighter(opts: HighlighterOptions) {
	if (!highlighter && !highlighterPromise) {
		...
		highlighterPromise = createHighlighter({ themes: ['github-dark', 'github-light'], langs }).then(h => {
			highlighter = h
		})
...
export async function initHighlighter({
	...
	if (!highlighter) {
		const loader = loadHighlighter({ languages })
		await loader
	} else {
		await Promise.all([...languages.flatMap(language => !!language && loadLanguage(language))])
	}
}
