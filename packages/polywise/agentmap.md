	...
	"structure": {
		...
		"src": {
			...
			"fst": {
				...
				"session": {
					"desc": "Session lifecycle and stream orchestration for chat and cron-triggered runs, consuming skill_map loading from fst/tools/skill, title_tool-based session renaming, and report_tool-based focus summaries only when the session is linked through todo_session; skill_map is normalized to name + description only, with skill file paths derived from skill name at runtime; supports archive/unarchive flow with archived_at state persisted in session_dir/state.json while running_since is persisted on the session table, and message queries scoped by archived boundary; appendMessage persists messages to database only, and trim/title/superego extraction are deferred after stream finish; successful stream completion now also resets is_runing through stop() so follow-up user messages are inserted normally, while todo-started sessions synchronize linked todo_session status transitions on finish, read, abort, and runtime failure. Session scope now distinguishes owner agent sessions from helper agents: only agent_session sets owner_agent and agent-scoped memory/wiki, while session_agent remains a helper list for task execution. session_dir/config.json stores disable_map and mode (normal|plan|plan-exec); plan mode disables write_file/edit_file tools and injects plan prompt; plan-exec mode auto-sends execution message after plan completes and resets mode to normal; clears session_dir/plan.md on message clear or archive",
					"role": "Folder"
				},
				"agents": {
				...
				"tools": {
					...
					"report.ts": "Internal tool that updates the short session report summary for todo-linked sessions, capped at 48 characters and guided to reflect user-visible progress during longer work with a mandatory final completed-style refresh before user-facing delivery and no further report_tool call after the final answer starts",
			...
			"rpc": {
				...
				"role": "Folder",
				"agent": {
					"desc": "Agent domain RPC procedures for create, remove, query, update, and drag-sort against the agent table, plus createSession and getSessions for agent-owned session creation and paginated listing through the dedicated agent_session relation",
					"role": "Folder"
				},
				...
				"session": {
					"desc": "Session subscriptions and management RPCs. Existing event bridge endpoints remain in events.ts; create/remove/rename/pin and group CRUD/sort/move, including moveOutGroup for removing a session id from a specific group by index, are split into dedicated single-file procedures. getList now returns grouped sessions by batch-loading group item ids and a separate first-page ungrouped session list, while getMoreList paginates additional ungrouped sessions in pages of 10 using the same group exclusion rule; both queries also exclude any session already linked through project_session so project-owned sessions never appear anywhere in the session page, and getList also excludes the reserved id global_panel_session from all query paths. Header session status data is now split between getSessionStatus for running, unread, and todo-error counts and getStatusList({ status }) for the active status list, where error membership is defined only by todo_session-linked todos whose status is error and non-IM sessions only. A session/types.ts file now centralizes the shared `running | unread | error` status union, while session/utils still holds single-file storage helpers, normalization helpers, and shared SessionGroupItem typing, and pin.json plus session_group.json path constants remain centralized in consts/app.ts; rename persists human title intent into context.json so title_tool no longer overrides manually renamed sessions. Replaced title-only watch with watchSessionStatus subscription payload `{ [session_id]: { title, report, running, unread, running_since, running_done } }`, added unread mutation endpoint to clear unread on enter, and unread-to-read now synchronizes linked todo status from unreview to done through todo_session when a session becomes read. Stream finish now marks unread=true when SessionEventStore has no `${id}/change` listeners, while manual stop and runtime failure flow through the same todo_session status machine in fst/session/stream. Session remove now enforces stream and event cleanup by aborting active session stream, unsubscribing SessionStreamStore, and clearing session-scoped SessionEventStore listeners before final store/file removal.",
					"role": "Folder"
				},
				"project": {
			...
			"db": {
				...
				"schema": {
					...
					"session.ts": {
						"desc": "Chat session storage schema with title, short report summary, nullable key, optional im/cron flags, running_since start timestamp, running_done end timestamp, and related indexes",
						"role": "Schema"
					},
					"project.ts": {
