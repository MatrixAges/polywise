...
- **Triple Extraction**: The `save()` and `update()` methods automatically extract subject-predicate-object triples from content using the ReBel model (DEFAULT_REBEL_MODEL) and inject them into the knowledge graph.
- **Pipeline**: Handles embedding, reranking, and triple extraction with configurable concurrency for each operation.
