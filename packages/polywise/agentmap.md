...
```json
{
	...
	"structure": {
		"src": {
			...
			"Pipeline.ts": {
				"desc": "Pipeline manager with embedding, reranking, and KeyBERT-based keyword extraction capabilities. Rerank output is normalized with label-aware score parsing (LABEL_1/relevant) for stable relevance scores. Keyword generation is exposed via generateKeywords() and leverages the existing embedding model.",
				"role": "Class"
			},
			"Polywise.ts": {
			...
			"utils": {
				...
				"migration.ts": {
					"desc": "Database schema migration system with version tracking and validation (CURRENT_SCHEMA_VERSION=2)",
					"role": "Module"
				}
			}
		},
...
- **Activation & Decay**: Implements biological Integrate-and-Fire mechanics with refractory periods and hyperpolarization. Weight decay is threshold-triggered based on input cumulative activity, preventing time-based loss of shared knowledge. Locked nodes/edges are preserved during decay.
