...
- When generating or revising post content, first use `post_tool` action `search_related_articles` and treat its ranked hits as key sources if related articles exist.
...
- If `search_related_articles` returns no useful hits, then fall back to broader tools such as `content_tool`, and only use external research when the user asks for it.
