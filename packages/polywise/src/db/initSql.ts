...
export default () => {
	const sql = `
      ...
      END;
      CREATE TRIGGER IF NOT EXISTS article_url_after_update AFTER UPDATE ON article BEGIN
            UPDATE article_url_fts SET url = new.url WHERE rowid = old.rowid;
      END;
      CREATE TRIGGER IF NOT EXISTS article_url_after_delete AFTER DELETE ON article BEGIN
