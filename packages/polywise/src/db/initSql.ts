...
export default () => {
	const sql = `
      ...
      -- article url fts 触发器
      CREATE TEMP TRIGGER article_after_insert AFTER INSERT ON article BEGIN
            INSERT INTO article_url_fts(rowid, url) VALUES (new.rowid, new.url);
      END;
      CREATE TEMP TRIGGER article_after_update AFTER UPDATE ON article BEGIN
            UPDATE article_url_fts SET url = new.url WHERE rowid = old.rowid;
      END;
      CREATE TEMP TRIGGER article_after_delete AFTER DELETE ON article BEGIN
            DELETE FROM article_url_fts WHERE rowid = old.rowid;
      END;

      -- chunk keywords fts 触发器
      CREATE TEMP TRIGGER chunk_after_insert AFTER INSERT ON chunk BEGIN
            INSERT INTO chunk_keywords_fts(rowid, keywords) VALUES (new.rowid, new.keywords);
      END;
      CREATE TEMP TRIGGER chunk_after_update AFTER UPDATE ON chunk BEGIN
            UPDATE chunk_keywords_fts SET keywords = new.keywords WHERE rowid = old.rowid;
      END;
      CREATE TEMP TRIGGER chunk_after_delete AFTER DELETE ON chunk BEGIN
