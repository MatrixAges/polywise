...
export const setAgentFrozenState = async (agent_id: string, is_frozen: boolean) => {
	...
	return env.db.transaction(async tx => {
		const [updated_agent] = await tx.update(agent).set({ is_frozen }).where(eq(agent.id, agent_id)).returning()

		await tx.update(node).set({ is_frozen }).where(eq(node.agent_id, agent_id))
		await tx.update(edge).set({ is_frozen }).where(eq(edge.agent_id, agent_id))
