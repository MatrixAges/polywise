...
export default (scope: ScopeInfo) => {
	const scope_desc =
		scope.scope_type === 'global'
			? 'Global scope (no project or agent binding)'
			: scope.scope_type === 'project'
				? `Project scope (project_id: ${scope.scope_id})`
				: `Agent scope (agent_id: ${scope.scope_id})`
