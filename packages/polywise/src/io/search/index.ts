...
export interface SerializedArticleScoreResult {
	article_id: string
	chunk_id: string
	rrf_score: number
	normalized_rrf_score: number
...
export interface SearchPipelineResult {
	...
	article_scores: Array<SerializedArticleScoreResult>
	output: SearchOutput
}
...
export const runSearchPipeline = async (args: ArgsRunPipeline): Promise<SearchPipelineResult> => {
	const {
		query,
		intent,
	...
	if (search_target_override) {
		...
		const combined_query = getSearchText(query, intent)
	...
	if (branch_results_override?.recall_chunk_ids || branch_results_override?.recall_article_ids) {
		...
		const recall_text = is_rewrite_mode
			...
			: getSearchText(query, intent)

		if (recall_text) {
			recall_result = await recall(recall_text, 'chunk', depth)
		}
...
export default async (args: ArgsSearch): Promise<SearchOutput> => {
	const pipeline_result = await runSearchPipeline(args)
