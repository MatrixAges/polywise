...
const buildBatchPrompt = (task: Pick<LinkcaseScheduleTask, 'count' | 'auto_remove_dead_links'>) => {
	if (task.auto_remove_dead_links) {
		return [
			...
			'Use smaller preview windows first when needed so you can locate the narrowest article body range before cleaning.',
			...
			'Before commit_preview, keep only the core article body in cleaned markdown.',
			'Do not paraphrase, summarize, translate, shorten, merge, reorder, or otherwise rewrite the article body.',
			'Keep the remaining body text verbatim from the fetched preview, except for minimal markdown or whitespace normalization that does not change wording.',
			'Use read_preview to narrow the body range first, then submit only the relevant body fragment instead of processing the whole cached preview.',
