  ...
  incoming_signals AS (
    SELECT
      ...
      SUM(
        ...
        * e.weight
        * ${system.tick.global_decay_rate}
        / (e.distance + 0.1)
      ) AS total_input
    FROM ${app.db.schema_brain}.edges e
    JOIN ${app.db.schema_brain}.nodes n ON n.id = e.source_id
...
const buildNodeInputUpdateSql = (inhibition_factor: number) => `
  ...
  UPDATE ${app.db.schema_brain}.nodes nd
  SET potential = LEAST(
    GREATEST(
      nd.potential + COALESCE(sig.total_input, 0) * (1 - ${inhibition_factor}),
      ${system.tick.min_potential}
    ),
    ${system.tick.tick_potential_max}
  )
...
const buildNodeStateUpdateSql = (threshold: number, threshold_decrement: number) => `
    ...
    SELECT
      ...
      (
        ...
        AND (
          ...
          OR EXTRACT(EPOCH FROM (CURRENT_TIMESTAMP - last_fired_at)) * 1000 > ${system.timing.refractory_period_ms}
        )
      ) AS can_fire
    FROM ${app.db.schema_brain}.nodes
  )
  ...
  SET
    potential = CASE
      ...
      ELSE nd.potential * ${system.tick.tick_decay_rate}
    END,
    current_threshold = CASE
      WHEN ev.is_active THEN LEAST(nd.current_threshold + ${system.shy.threshold_spike}, ${system.shy.max_threshold})
      ELSE GREATEST(nd.current_threshold - ${threshold_decrement}, ${system.shy.min_threshold}, ${threshold})
    END,
    transmitter = CASE
...
const buildEdgeLearningUpdateSql = (arousal: number) => `
    ...
    SELECT
      ...
      CASE
        ...
        WHEN source_active AND target_active THEN LEAST(
          weight + (${system.node_edge.edge_learning_factor} * learning_rate * ${arousal} * (1 - weight / ${system.node_edge.edge_weight_max})),
          ${system.node_edge.edge_weight_max}
        )
        WHEN source_active OR target_active THEN GREATEST(
          weight - (${system.node_edge.edge_decay_factor} * learning_rate),
  ...
  SET
    ...
    distance = GREATEST(1.0 / (nx.next_weight + ${system.tick.distance_epsilon}), 0.1),
...
export const sql_decay = `
  ...
  SET weight = GREATEST(weight - (${system.node_edge.edge_decay_factor} / decay_resistance), ${system.node_edge.edge_weight_min}),
      distance = GREATEST(1.0 / (GREATEST(weight - (${system.node_edge.edge_decay_factor} / decay_resistance), ${system.node_edge.edge_weight_min}) + ${system.tick.distance_epsilon}), 0.1)
 ...
 */
export const sql_stimulate = `UPDATE ${app.db.schema_brain}.nodes SET potential = potential + $1 WHERE id = $2`
