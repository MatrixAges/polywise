...
export const sql_get_snapshot_nodes = (weight_threshold: number, limit: number) => `
  ...
  WHERE potential > ${system.node_edge.node_potential_min}
  OR id IN (SELECT source_id FROM ${app.db.schema_brain}.edges WHERE weight > ${weight_threshold})
  OR id IN (SELECT target_id FROM ${app.db.schema_brain}.edges WHERE weight > ${weight_threshold})
