...
describe('Polywise Brain System', () => {
	...
	describe.concurrent('Complex Knowledge Graph Operations', () => {
		it(
			'should process complex software architecture text',
			async () => {
		...
		it(
			'should process complex scientific text with real-world knowledge',
			async () => {
		...
		it(
			'should handle large scale knowledge network with real entities',
			async () => {
	...
	describe.concurrent('Brain Dynamics and Learning', () => {
		it(
			'should propagate activation through network chains using real entities',
			async () => {
				const chain: number[] = []
				const entities = [
					'Processor',
		...
		it(
			'should handle complex learning burst with fatigue using network hierarchy',
			async () => {
				const concepts = [
		...
		it(
			'should maintain and strengthen important connections',
			async () => {
				await poly.save({
	...
	describe.concurrent('Schema and Data Integrity', () => {
		it(
			'should handle concurrent node operations',
			async () => {
				const promises: Promise<number>[] = []

				for (let i = 0; i < 15; i++) {
					promises.push(
						poly.addNode({ label: `Concurrent_${i}`, x: i * 20, y: i * 10, threshold: 0.5 })
					)
				}
	...
	describe.concurrent('Node and Edge Filtering with idol_id, root_ids, metrics_ids', () => {
		it(
			'should create nodes with idol_id and filter by idol',
			async () => {
		...
		it(
			'should filter nodes by root_id',
			async () => {
		...
		it(
			'should filter edges by idol_id',
			async () => {
		...
		it(
			'should filter edges by root_id',
			async () => {
		...
		it(
			'should process article with filtering metadata',
			async () => {
		...
		it(
			'should include new fields in snapshot',
			async () => {
	...
	describe.concurrent('Article CRUD and Search', () => {
		it(
			'should add article and retrieve by id',
			async () => {
		...
		it(
			'should add article with embedding',
			async () => {
		...
		it(
			'should search articles by vector similarity',
			async () => {
				await poly.article.addWithEmbedding(
