	...
	describe('Intelligence & Capabilities', () => {
		describe('Logic & Reasoning', () => {
			it('should solve simple boolean logic', async () => {
				const res = await pipeline.decide(datasets_decision.prompt_boolean_logic, { max_new_tokens: 5 })
		...
		describe('Linguistics & Semantics', () => {
			it('should detect synonyms', async () => {
				const res = await pipeline.decide(datasets_decision.prompt_synonyms, { max_new_tokens: 10 })
				expect(res.toUpperCase()).toContain('YES')
			})
			...
			it('should identify sentiment', async () => {
				const res = await pipeline.decide(datasets_decision.prompt_sentiment, { max_new_tokens: 10 })
		...
		describe('Instruction Following', () => {
			...
			it('should format output as requested', async () => {
				const res = await pipeline.decide(datasets_decision.prompt_json_format, { max_new_tokens: 20 })
			...
			it('should answer with single word', async () => {
				const res = await pipeline.decide(datasets_decision.prompt_single_number, { max_new_tokens: 5 })
